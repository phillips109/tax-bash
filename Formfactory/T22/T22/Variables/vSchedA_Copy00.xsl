...
<xsl:variable name="SchedA-Copy00-line75" select="0.075"/>
...
<xsl:variable name="SchedA-Copy00-line02">
  <xsl:choose><xsl:when test="document('../Output/Form1040_Copy00.xml')">
    ...
    </xsl:when><xsl:otherwise><xsl:value-of select="200"/></xsl:otherwise></xsl:choose></xsl:variable>
...
<xsl:choose>
<xsl:when test="$SchedA-Copy00-lineFS = MFS">
