...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="tlin">
    <xsl:variable name="file" select="concat('../Data/',$form, '_Copy',$copy, '_line', @name, '_', 'text', '.xml')"/>
	...
	<xsl:value-of select="@color"/>
      ...
      <linename>
	<xsl:value-of select="$form"/>-Copy<xsl:value-of select="$copy"/>-line<xsl:value-of select="@name"/>
      </linename>
      <line>
	...
	<xsl:value-of select="instructions"/>
      ...
      <IRSinstructions>
	<xsl:value-of select="IRSinstructions"/>
      </IRSinstructions>
      <notes>
      </notes>
      <text>
  ...
  <xsl:template match="dlin">
	...
	<xsl:value-of select="@color"/>
      ...
      <linename>
	<xsl:value-of select="$form"/>-Copy<xsl:value-of select="$copy"/>-line<xsl:value-of select="@number"/>
      </linename>
  ...
  <!-- Note two versions of tPull. Not sure which is right. -->
  <!-- although we have tPull as well as tlin entries, only tlin is made into a ../Data/ file. -->
  ...
  <!-- Note that we now have dPull as well as dlin entries, only dlin are made into ../Data/files. -->
  
<!-- Note two versions of dPull. Not sure which is right. -->
